...
public interface TwoPointGradingRepository extends JpaRepository<TwoPointGrading, Long> {
    ...
    @Query(value="select g from TwoPointGrading g join fetch g.staff s where s.staffId = ?1 order by g.name asc")
    Slice<TwoPointGrading> findAllByStaffId(Long staffId, Pageable pageable);

    @Query(value="select g from TwoPointGrading g join fetch g.staff s join s.department d where d.depId = ?1 order by g.name asc")
    Slice<TwoPointGrading> findAllByDepartmentId(Long depId, Pageable pageable);

    @Query(value="select g from TwoPointGrading g join fetch g.staff s where s.staffId = ?1 and g.name like %?2% order by g.name asc")
    Slice<TwoPointGrading>  findAllByStaffIdAndNameLettersContains(Long staffId, String letters, Pageable pageable);

    @Query(value="select g from TwoPointGrading g join fetch g.staff s join s.department d where d.depId = ?1 and g.name like %?2% order by g.name asc")
    Slice<TwoPointGrading> findAllByDepartmentIdAndNameLettersContains(Long depId, String letters, Pageable pageable);

    //-------------------------------------------ADMIN----------------------------------------

    @Query(value="select g from TwoPointGrading g join fetch g.staff s order by g.name asc")
