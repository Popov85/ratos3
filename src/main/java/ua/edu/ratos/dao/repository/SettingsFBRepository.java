...
public interface SettingsFBRepository extends JpaRepository<SettingsFB, Long> {
    ...
    @Query(value="select s from SettingsFB s join fetch s.lang join fetch s.staff st where st.staffId = ?1")
    Slice<SettingsFB> findAllByStaffId(Long staffId, Pageable pageable);

    @Query(value="select s from SettingsFB s join fetch s.lang join fetch s.staff st join st.department d where d.depId = ?1")
    Slice<SettingsFB> findAllByDepartmentId(Long depId, Pageable pageable);

    //------------------------------------------------------Search------------------------------------------------------

    @Query(value="select s from SettingsFB s join fetch s.lang join fetch s.staff st where st.staffId = ?1 and s.name like %?2%")
    Slice<SettingsFB> findAllByStaffIdAndNameLettersContains(Long staffId, String letters, Pageable pageable);

    @Query(value="select s from SettingsFB s join fetch s.lang join fetch s.staff st join st.department d where d.depId = ?1 and s.name like %?2%")
