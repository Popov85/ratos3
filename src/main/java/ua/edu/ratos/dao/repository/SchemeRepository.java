...
public interface SchemeRepository extends JpaRepository<Scheme, Long> {
    ...
    @Query(value = "SELECT s FROM Scheme s join fetch s.strategy join fetch s.settings join fetch s.mode join fetch s.grading join fetch s.course join fetch s.staff st join fetch st.user join fetch st.position join st.department d join fetch s.access left join fetch s.themes left join fetch s.groups where d.depId =?1 order by s.name asc",
            countQuery = "SELECT count(s) FROM Scheme s join s.staff st join st.department d where d.depId=?1")
    Page<Scheme> findAllByDepartmentId(Long depId, Pageable pageable);

    @Query(value = "SELECT s FROM Scheme s join fetch s.strategy join fetch s.settings join fetch s.mode join fetch s.grading join fetch s.course join fetch s.staff st join fetch st.user join fetch st.position join st.department d join fetch s.access left join fetch s.themes left join fetch s.groups where d.depId =?1 and s.name like %?2% order by s.name asc",
            countQuery = "SELECT count(s) FROM Scheme s join s.staff st join st.department d where d.depId=?1 and s.name like %?2%")
