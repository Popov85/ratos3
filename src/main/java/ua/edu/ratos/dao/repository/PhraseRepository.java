...
public interface PhraseRepository extends JpaRepository<Phrase, Long> {
    ...
    @Query(value = "SELECT p FROM Phrase p join fetch p.staff s left join fetch p.resources where s.staffId =?1 order by p.lastUsed desc",
    ...
    @Query(value = "SELECT p FROM Phrase p join fetch p.staff s left join fetch p.resources join s.department d where d.depId =?1 order by p.lastUsed desc",
    ...
    @Query(value = "SELECT p FROM Phrase p join p.staff s left join fetch p.resources where s.staffId =?1 and p.phrase like %?2% order by p.phrase asc",
    ...
    @Query(value = "SELECT p FROM Phrase p join fetch p.staff s left join fetch p.resources join s.department d where d.depId =?1 and p.phrase like %?2% order by p.phrase asc",
    ...
    // ----------------------------------------------ADMIN---------------------------------------------
    @Query(value="SELECT p FROM Phrase p join fetch p.staff left join fetch p.resources order by p.phrase asc",
