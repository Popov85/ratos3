...
public class LTIAuthenticationHandler implements OAuthAuthenticationHandler {
    ...
    public Authentication createAuthentication(HttpServletRequest request, ConsumerAuthentication authentication, OAuthAccessProviderToken authToken) {
        ...
        if (lms!=null) {
            lmsId = lms.getLmsId();
        } else {// Fallback value if LMS is not found by consumerKey
            lmsId = 1L;
        }
        ...
        if (email!=null && !email.isEmpty()) {
            ...
            if (user!=null) {
                ...
                log.debug("LTI user authentication successful after launch request :: {}", auth);
                return auth;
            }
        }
        ...
        log.debug("LTI LMS authentication successful after launch request, still needs re-authentication to access protected resources :: {}", auth);
        return auth;
    }
}
